...
// Pantalla principal: lista de productos de las concesiones cargada desde la API.
    ...
    >
      ...
      <View style={styles.cardContent}>
        ...
        <Text style={styles.badge}>
          ${item.price} · {item.type} · {item.available ? 'Disponible' : 'Agotado'}
        </Text>
      </View>
...
export function HomeScreen(): React.JSX.Element {
  ...
  const { data, isLoading, isError, isFetching, refetch, error } = useConcessions();
  ...
  const renderItem: ListRenderItem<ConcessionItem> = ({ item }) => (
    <ItemCard
      item={item}
...
const styles = StyleSheet.create({
  ...
  badge: { ...TYPOGRAPHY.caption, color: COLORS.accent, fontWeight: '600' },
