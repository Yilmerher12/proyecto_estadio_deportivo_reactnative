...
// Custom hooks para CRUD de productos de las concesiones usando TanStack Query + Axios.
...
// ============================================================
// Forma real de un post de JSONPlaceholder — proxy de red mientras
// no hay un backend propio desplegado (igual que en semana 05).
// ============================================================
...
// Catálogo fijo de productos del estadio. Cada post se mapea de forma
// determinística (por su id) a uno de estos productos.
...
// ─────────────────────────────────────────
// READ — producto individual (para el formulario Edit)
// ─────────────────────────────────────────
...
export function useUpdateConcession() {
  const queryClient = useQueryClient();
  return useMutation<ConcessionItem, Error, UpdateConcessionPayload>({
    mutationFn: async (payload) => {
      await apiClient.put<UpdateConcessionPayload>(`/posts/${payload.id}`, payload);
      return {
        id: payload.id,
        name: payload.name,
        description: payload.description,
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: CONCESSIONS_QUERY_KEY });
      queryClient.invalidateQueries({ queryKey: [...CONCESSIONS_QUERY_KEY, variables.id] });
    },
  });
}
